...
//[MINDFUL] con trỏ listener + hàm "nghe lén" 1 từ hoàn chỉnh
void (*vOnWordCommitted)(const wstring& word) = NULL;

static void emitCommittedWord() {
    if (vOnWordCommitted == NULL || _index <= 0)
        return;
    wstring w;
    for (int i = 0; i < _index; i++) {
        Uint32 c = getCharacterCode(TypingWord[i]);
        Uint32 cp;
        if (c & PURE_CHARACTER_MASK)    cp = c & 0xFFFF;
        else if (!(c & CHAR_CODE_MASK)) cp = keyCodeToCharacter(c);
        else                            cp = c & 0xFFFF;
        if (cp) w.push_back((wchar_t)cp);
    }
    if (!w.empty())
        vOnWordCommitted(w);
}
...
void startNewSession() {
    emitCommittedWord(); //[MINDFUL] nghe từ vừa xong trước khi reset
    _index = 0;
    hBPC = 0;
    hNCC = 0;
    tempDisableKey = false;

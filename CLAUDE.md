...
| 2026-07-11 | Thêm skill `.claude/skills/mindful-test-design/` (SKILL + 2 reference: `telex-vni-edge-cases.md`, `test-done-checklist.md`) — chắt lọc từ Anh Tester `antigravity/claude-testing-kit` (MIT, giữ credit). Lấy 4 giá trị đã dịch sang bàn phím native: ma trận ca biên Telex/VNI (từ Field-Level Validation), Risk-Based Testing theo rủi ro dự án, Definition of Done cho test (`make test`/`xcodebuild`/CI + ca iOS chập chờn PASS 2 lần), "verify đừng đoán". | So sánh 2 testing-kit: ~40% áp được cho bàn phím native, 60% là đồ web/DOM/Playwright/Jira → **distill không bê nguyên** (tránh cargo-cult domain web + khỏi kéo dependency lạ). Gia cố lưới an toàn cho engine đúng lúc Mốc B (nối `vKeyHandleEvent` gõ Telex ra dấu) sắp đụng tới. |

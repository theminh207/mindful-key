...
//
// TIẾNG CHUÔNG NHÚNG THẲNG VÀO .EXE: OpenKey.rc trỏ sang chính tệp .wav mà vỏ macOS đang dùng
...
static void PlayBellWav(const wstring& sid, float vol) {
    ...
    if (key != g_cacheKey || g_soundCache.empty()) {
        ...
        if (sid == kSoundIdCustom) {
            wstring p = OpenKeyHelper::getRegString(kRegCustomPath, _T(""));
...
static void playBellSound() {
    wstring sid = SoundIdFromStored(OpenKeyHelper::getRegString(kRegSoundName, _T("")));
    if (sid == kSoundMute)
        return;   // "Im" — không phát gì, có chủ đích

    int pct = OpenKeyHelper::getRegInt(kRegVolume, 60);   // 60% = macOS mặc định 0.6
...
bool Bell_InstallCustomSound(LPCTSTR path, wstring* outMessage) {
    ...
    OpenKeyHelper::setRegString(kRegCustomPath, dst.c_str());
    OpenKeyHelper::setRegString(kRegSoundName, kSoundIdCustom);
...
static INT_PTR CALLBACK BellDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG: {
        ...
        SetDlgItemInt(hDlg, IDC_EDIT_BELL_VOLUME, OpenKeyHelper::getRegInt(kRegVolume, 60), FALSE);
    ...
    case WM_COMMAND:
        if (LOWORD(wParam) == IDC_BUTTON_BELL_PREVIEW) {
            ...
            OpenKeyHelper::setRegInt(kRegVolume, vol);
            ...
            if (sel >= 0 && sel < kSoundComboCount)
                OpenKeyHelper::setRegString(kRegSoundName, ids[sel]);

...
bool MoodStore_HasConsent() {
    return OpenKeyHelper::getRegInt(kRegConsent, 0) != 0;
}

bool MoodStore_HasAskedConsent() {
    return OpenKeyHelper::getRegInt(kRegAskedConsent, 0) != 0;
}

void MoodStore_SetConsent(bool granted) {
    OpenKeyHelper::setRegInt(kRegConsent, granted ? 1 : 0);
    OpenKeyHelper::setRegInt(kRegAskedConsent, 1);
    if (!granted)

...
void MindfulKeyInit() {
	...
	APP_GET_DATA(vTempOffMindfulKey, 0);
	...
	//   MindfulKeyInit() chạy ở AppDelegate::run() dòng ~98 — TRƯỚC createSystemTrayIcon() (~101) và
	...
	DWORD macroDataSize;
	BYTE* macroData = MindfulKeyHelper::getRegBinary(_T("macroData"), macroDataSize);
	initMacroMap((Byte*)macroData, (int)macroDataSize);

	//init and load smart switch key data
	DWORD smartSwitchKeySize;
	BYTE* data = MindfulKeyHelper::getRegBinary(_T("smartSwitchKey"), smartSwitchKeySize);
...
void saveSmartSwitchKeyData() {
	...
	MindfulKeyHelper::setRegBinary(_T("smartSwitchKey"), savedSmartSwitchKeyData.data(), (int)savedSmartSwitchKeyData.size());
}
...
static void SendBackspace() {
	...
	if (vSupportMetroApp && MindfulKeyHelper::getLastAppExecuteName().compare("ApplicationFrameHost.exe") == 0) {//Metro App
...
static void SendNewCharString(const bool& dataFromMacro = false) {
	...
	MindfulKeyHelper::setClipboardText((LPCTSTR)_newCharString.data(), _newCharSize + 1, CF_UNICODETEXT);
...
LRESULT CALLBACK keyboardHookProcess(int nCode, WPARAM wParam, LPARAM lParam) {
	...
	// Hạn chế đã biết, giống hệt bản macOS (xem MindfulKey.mm): từ CUỐI ngay trước Enter chưa được
	...
	if ((wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN) && !_isFlagKey && _keycode != 0) {
		...
		if (_lastFlag == 0 || _lastFlag < _flag)
			...
			if (vTempOffMindfulKey && !_hasJustUsedHotKey && _lastFlag & MASK_ALT) {
				vTempOffEngine();
			}
			_lastFlag = _flag;
		...
		if (pData->code == vDoNothing) { //do nothing
			...
			if (vFixRecommendBrowser && pData->extCode != 4) {
				if (vFixChromiumBrowser && 
					std::find(_chromiumBrowser.begin(), _chromiumBrowser.end(), MindfulKeyHelper::getLastAppExecuteName()) != _chromiumBrowser.end()) {
...
VOID CALLBACK winEventProcCallback(HWINEVENTHOOK hWinEventHook, DWORD dwEvent, HWND hwnd, LONG idObject, LONG idChild, DWORD dwEventThread, DWORD dwmsEventTime) {
	...
	if (vUseSmartSwitchKey || vRememberCode) {
		string& exe = MindfulKeyHelper::getFrontMostAppExecuteName();

...
// dwExtraInfo != 0 (MindfulKey.cpp:499) — cùng vai trò `myEventSource` bên macOS: không có nó thì
...
void SendGatekeeper_Init() {
    wstring raw = MindfulKeyHelper::getRegString(kAppsRegKey, _T(""));
...
static bool IsFrontmostAppWatched() {
    ...
    string utf8 = MindfulKeyHelper::getFrontMostAppExecuteName();
    if (utf8.empty())
        return false;
...
wstring SendGatekeeper_LastAppName() {
    string utf8 = MindfulKeyHelper::getLastAppExecuteName();
...
void SendGatekeeper_ToggleLastApp() {
    ...
    MindfulKeyHelper::setRegString(kAppsRegKey, joined.c_str());

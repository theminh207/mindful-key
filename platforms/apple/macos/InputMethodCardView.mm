...
static const CGFloat kHeaderH = 20.0;    // hàng "● Bộ gõ … ▸"
...
- (instancetype)initWithFrame:(NSRect)frameRect {
    if ((self = [super initWithFrame:frameRect])) {
        ...
        // Toggle gõ hằng ngày — key/global khớp đúng IBAction ViewController.
        _toggleDefs = @[
            @{@"t":@"Kiểm tra chính tả",        @"k":@"Spelling",           @"spell":@YES},
            @{@"t":@"Đặt dấu oà, uý (kiểu mới)", @"k":@"ModernOrthography"},
...
- (void)applyCollapsedState {
    ...
    NSMutableArray<NSView *> *fields = [@[_lblType, _typePopup, _lblCode, _codePopup, _lblVN, _vnSwitch, _fullLink] mutableCopy];
...
- (CGFloat)preferredHeight {
    if (_collapsed && !_hideHeaderRow) return 48.0;
    CGFloat h = kPad;
    if (!_hideHeaderRow) h += kHeaderH + kFieldGap;
...
- (void)layout {
    ...
    CGFloat top;
    if (_hideHeaderRow) {
        // [MINDFUL] popover 3-tab: tab đã có nhãn "Bộ gõ" rồi, bỏ hàng tiêu đề + luôn bung
        ...
        top = kPad + kHeaderH + kFieldGap;   // khoảng cách tính từ đỉnh
    }

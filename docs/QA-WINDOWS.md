...
| # | Bước | Phải xảy ra |
|---|---|---|
| J0 | Cài mới, chạy lần đầu | Hiện hộp hỏi **một lần**: "Bật nhật ký cảm xúc?" — nội dung nói rõ không chứa nội dung gõ |
| J1 | Tắt app, mở lại | **KHÔNG hỏi lại** — hỏi 1 lần trong đời, không phải mỗi lần khởi động |
| J2 | Chọn **Không** ở J0 → gõ `ddm` + cách → Enter (app đã gác) → chọn gì đó | Tệp `mood.enc` **không tồn tại**. Không đồng ý = không ghi gì |
...
> 🔴 **P1 GẦN NHƯ CHẮC CHẮN FAIL — và đây là lỗi CHẶN PHÁT HÀNH, không phải "cải thiện sau".**
>
> Điều tra 2026-07-17 (sửa lại chính câu tôi viết sai ở bản trước — tôi từng ghi "bản macOS/iOS có"
> mà không kiểm):
>
...
> | **Windows** | **KHÔNG GÌ CẢ.** `WH_KEYBOARD_LL` thấy HẾT. Windows không có cơ chế nào chặn hook như macOS |
>
> Nghĩa là **Windows là vỏ DUY NHẤT mà lớp cảm xúc đọc mật khẩu người dùng**: chữ vào `MoodBuffer`,
> bị chấm điểm, có thể bật lời nhắc giữa lúc đăng nhập, và nhịp đó có thể vào nhật ký (chỉ điểm số
> + tên app, không phải chữ — nhưng vẫn là đã ĐỌC).
>
> Đây không phải "chưa làm cho đẹp": nền tảng này không cho không thứ mà macOS được cho không.
> Chạm CỘT TRỤ RIÊNG TƯ của hiến chương.

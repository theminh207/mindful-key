...
· F1 điều tra mục "Hệ thống" trắng trơn · F13 gỡ 2 UI Chuông đá nhau · F15 bố cục Bộ gõ · F16
căn giữa icon Bộ tiếng · F2/F3/F4 hồn của "Hôm nay" — **[2026-07-15] PHÌNH SCOPE, đã shard
thành 4 story đầy đủ 3.5–3.8** (xem dưới) sau khi soát kỹ phát hiện việc lớn hơn dự tính ban đầu
(cần viết mới cả tầng dữ liệu Tuần/Tháng, không chỉ nối dây có sẵn).

**Out of scope:** PhoBERT + tín hiệu "gõ thế nào" (vẫn là lộ trình xa). ~~F9/F10~~ đã chốt
2026-07-15 (xem `decision-log.md` "Chốt 3 câu chặn Epic 3" — tên "Mindful Key", chuông
30/60+tùy chỉnh có sàn 15).

**Cross-epic dependencies:**
- Blocked by: Epic 2 (thi công xong mới nghiệm thu được).
- Blocks: coi Epic 2 là đóng.

> Story 3.1/3.2/3.4 KHÔNG có file `.story.md` đầy đủ — cùng kiểu quyết định có chủ đích như
> 1.7–1.10: spec sống ở `ACCEPTANCE-v2-2026-07-15.md` + gate 1/2/4/5/6/7 trong `sprint-status.yaml`
> → `epic3_acceptance` (việc nhỏ, rủi ro thấp, không cần overhead viết đủ format).
>
> **Story 3.3 (placeholder "today-pane-gatekeeper-river-reflection") ĐÃ ĐƯỢC THAY THẾ** bởi 4
> story đầy đủ, format chuẩn BMAD — soát kỹ lúc chủ dự án yêu cầu "lên kế hoạch triển khai" cho
> thấy việc này đủ lớn/đủ rủi ro nhận diện để xứng đáng rigor cao hơn các gate nhẹ khác:
> - `3.5.gatekeeper-card-and-reflection-link` — NHỎ, tái dùng 100% code có sẵn (`GatekeeperCardView`).
> - `3.6.reflection-screen-real-river` — VỪA, vẽ lại theo mockup A1 + gỡ số thô "Đỉnh gợn hôm nay".
> - `3.7.week-river-view` — LỚN, viết mới `MoodStoreMac_FetchWeekSamples` + UI Ngày/Tuần.
> - `3.8.month-river-view` — LỚN + **rủi ro nhận diện cao nhất** (chưa có mockup tham chiếu nào,
>   phải DỪNG-HỎI chủ dự án về hình dạng hiển thị trước khi code — xem story, mục "Câu hỏi thiết kế").
>
> **PHẢI làm TUẦN TỰ 3.5→3.6→3.7→3.8**, không song song — `scope-conflict-check.sh` xác nhận cả
> 4 story chồng lấn `SettingsWindowController.mm` (pane "Hôm nay") và 3.7/3.8 còn chồng thêm
> `MoodStoreMac.h/.mm` (2026-07-15, 4 conflict phát hiện, khớp đúng dự đoán lúc viết story).

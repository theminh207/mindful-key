...
- **Rationale:** Đối chiếu cấu trúc: lý thuyết ngoài là mô hình 2 làn (+/-) mà "điềm tĩnh" =
  CÙNG con sóng đó nhưng biên độ nhỏ lại (không phải dừng hẳn), đích tới là "thấy được cả 2
  phía". Mô hình hiện tại của app chỉ đo "đang động nhiều hay ít" (1 trục không dấu), KHÔNG chấm
  câu vừa gõ là tích cực hay tiêu cực — đây là hệ quả TRỰC TIẾP của điều tối kỵ §2.2 ("KHÔNG đèn
  đỏ/xanh mã hóa cảm xúc, đỏ=xấu xanh=tốt"): gắn nhãn +/- cho từng khoảnh khắc chính là phán xét,
  dù tinh vi hơn đèn giao thông. Đối chiếu 2 mô hình: khớp ~85-90% với hiến chương CỦA DỰ ÁN (đặc
  biệt "bình tĩnh = bớt dao động" được biên độ→0 nắm khá tốt), nhưng chỉ ~35-45% khớp với ĐÚNG sơ
  đồ lý thuyết ngoài (thiếu hẳn trục +/- và khái niệm "thấy cả 2 phía"). Được trình bày rõ 2 con
  số này cho chủ dự án trước khi hỏi, KHÔNG tự chọn giữ nguyên trong im lặng.
- **Alternatives considered:** (B) mở rộng câu mô tả "hình dạng cả ngày" đã có sẵn
  (`MoodPhrasing_DayShapeSentence`) để gợi ý "ngày có lúc động có lúc lặng" — rủi ro thấp, không
  đụng UI/màu, gần tinh thần "thấy cả 2 phía" hơn mà không gắn nhãn tốt/xấu. (C) dựng hẳn trục
  +/- — thay đổi cấu trúc nhận diện, cần phiên thiết kế riêng (rủi ro trượt thành chấm điểm cảm
  xúc). Chủ dự án chọn KHÔNG làm B lẫn C ở lượt này — giữ nguyên, không sửa code.
- **Made by:** Claude Code (Fable 5), qua AskUserQuestion trực tiếp với chủ dự án
- **Supersedes:** none — củng cố thêm cho §2.1-2.3 đã có, không đổi luật cũ

---

### 2026-07-17 — Icon ô thứ 4 = NỐT NHẠC, không phải chuông (chủ dự án nhìn app thật rồi bác)
- **Phản chứng bằng mắt:** chủ dự án cài bản mới, chụp thẻ "Bộ tiếng" và bác hình `bell-idle`: nó là
  **chuông trơn** nên nằm cạnh ô "Chuông gió" (`bell_chime`, cũng chuông trơn) thì đọc ra **trùng
  icon**, và không nói được ý "tự chọn". Yêu cầu: "1 icon khác để không bị trùng icon mà mang tính
  tự chọn được. Có thể là icon ?"
- **Quyết định:** ô thứ 4 dùng **nốt nhạc** — asset MỚI `brand/svg/bell-custom.svg`, vẽ theo đúng
  nét họ brand (teal `#1D7C91`, stroke 6, đầu bo tròn; không dùng cam vì cam chỉ cho CTA/link active
  — DESIGN.md §5 điều 6). `bell-idle.png/@2x` gỡ khỏi `platforms/apple/macos/` (hết chỗ dùng).
- **Rationale:** 3 phương án được vẽ thật rồi ghép vào đúng hàng 4 ô cho chủ dự án xem tận mắt
  (nốt nhạc · dấu + trong vòng nét đứt · dấu `?`). Chủ dự án chọn nốt nhạc. Hai phương án kia bị
  loại có lý do: **`?`** (chính gợi ý của chủ dự án — vẫn vẽ ra để xem thật, không cãi suông) trong
  hàng đọc ra "chuông bí ẩn / trợ giúp", không phải "tiếng của tôi"; **`+`** chỉ đúng khi CHƯA nạp
  gì — nạp tệp rồi mà ô vẫn hiện `+` thì vô lý, còn nốt nhạc đúng ở cả 2 trạng thái. (Phương án
  "đổi hình theo trạng thái: `+` → nốt nhạc" có đưa ra, chủ dự án không chọn — 2 asset + 1 nhánh
  trạng thái để đổi lấy quá ít.)
- **Bài học:** đây là lần thứ 2 trong ngày mắt-nhìn-app-thật bác một thứ mà build/test/brand-lint
  đều xanh. Icon `bell-idle` đã được chọn với lý do "asset brand có sẵn, không vẽ mới" — đúng luật,
  nhưng sai nghĩa; và cái sai chỉ lộ ra khi đặt nó CẠNH 3 ô kia, không lộ khi nhìn file rời.
- **Supersedes:** "Quyết định 1" của entry ngay dưới (phần chọn `bell-idle`) — phần còn lại của
  entry đó (có ô thứ 4, id tiếng Anh, chép tệp vào kho app) VẪN nguyên hiệu lực.
- **Made by:** phiên sửa lỗi + cải tiến theo phản hồi người dùng thật (skill `platform-porting`).

---

### 2026-07-17 — Bộ tiếng có Ô THỨ 4 "tiếng của bạn"; kho lưu đổi sang id tiếng Anh
- **Bối cảnh:** người dùng thật (MacBook) báo 3 tiếng chuông mặc định không đúng bảng phát triển.
  Truy ra 2 việc: (1) bug mặc định — `vBellSoundName` chưa set thì `BellSettingsView` sáng "Chuông
  chùa" còn `playBellSound` phát "Glass" (ping hệ thống macOS, chưa bao giờ là thiết kế), đã vá ở
  `189eef7`; (2) chủ dự án muốn thêm khả năng **tự nạp tiếng chuông riêng**.
- **Quyết định 1 — thêm ô thứ 4 vào hàng icon "Bộ tiếng"** (chủ dự án chọn trực tiếp giữa 2 phương
  án được đưa ra: "dòng riêng dưới 3 icon" vs "icon thứ 4 trong hàng"). Bấm ô thứ 4 = mở hộp chọn
  tệp; hình dùng asset brand **có sẵn** `bell-idle` (`brand/svg/bell-idle.svg`), KHÔNG vẽ hình mới.
- **Supersedes:** `bmad-output/macos/stories/2.5.bell-detail-sounds.story.md` §Dev Notes ("Bộ tiếng —
  giữ nguyên 3 lựa chọn theo mockup, KHÔNG thêm ô thứ 4") và `bmad-output/macos/SCREEN-REFERENCE.md`
  §2.2 + §5 ("3 icon brand GIỮ NGUYÊN … TUYỆT ĐỐI không thay bằng icon ngoài"). Cả 2 file đã được
  dán banner SUPERSEDED cùng lượt. Lý do đè: 2 văn bản đó khoá "3 ô" theo `mockup-v2-tabbed.html`,
  nhưng mockup vẽ khi CHƯA có yêu cầu chuông riêng — chủ dự án là người quyết cuối trên sản phẩm.
  Phần còn lại của cả 2 file (ánh xạ tag→tiếng, `_bellIndicator`, cấm icon ngoài brand) VẪN HIỆU LỰC.
- **Quyết định 2 — `vBellSoundName` lưu ID TIẾNG ANH** (`temple`/`chime`/`wind`/`custom`/`__silent__`)
  thay vì nhãn tiếng Việt ("Chuông chùa"). Lý do: (a) CLAUDE.md "định danh = tiếng Anh, UI = tiếng
  Việt" — giá trị này là thứ máy đọc; (b) tiếng riêng của người dùng KHÔNG có "tên" để lưu, chỉ có
  đường dẫn tệp → mô hình cũ kẹt cứng. **Không có bước migrate**: `BellMac_SoundIdFromStored()` nuốt
  cả dữ liệu đời cũ lẫn rác ("Glass"/"Tink" thời placeholder) → người đang dùng không mất tiếng đã
  chọn. Khoá test: `tests/macos` Ca 8 (đã verify RED thật).
- **Quyết định 3 — CHÉP tệp người dùng vào kho app** (`~/Library/Application Support/MindfulKeyboard/
  CustomBell.<đuôi>`, cùng thư mục `mood.enc`; `MoodStoreMac_DeleteAll` chỉ xoá đúng `mood.enc` nên
  không dọn nhầm) thay vì giữ đường dẫn gốc: xoá tệp nguồn / rút USB thì chuông vẫn phải kêu. Tệp
  riêng hỏng-mất → rơi về tiếng THIẾT KẾ (không rơi về ping hệ thống), và màn Chuông sáng đúng ô sẽ
  thật sự kêu. Tiếng khung chấm nhịp vẫn CỐ ĐỊNH "Chuông reo", không theo tiếng riêng.
- **Made by:** phiên sửa lỗi + cải tiến theo phản hồi người dùng thật (skill `platform-porting`).

---

### 2026-07-16 — "Sông không có dòng" KHÔNG phải lỗi view — dữ liệu giả lập sai bản chất (lỗi tự gây)
- **Triệu chứng:** chủ dự án khoanh đỏ ảnh popover: tab "Hôm nay" chỉ có **chấm trôi lơ lửng, không
  có dòng nào**, chấm dồn cục vô nghĩa quanh Trưa/Chiều. Yêu cầu: *"kiểm tra dòng cảm xúc hôm nay
  tracking từng khung trạng thái đang chạy theo nhịp sóng"* + *"sửa lại dòng đo đường. Cân chỉnh rõ ràng"*.
- **Chẩn:** `EmotionRiverView` **làm đúng luật** — thủ phạm là `MoodStoreMac_SeedFakeSamplesForTesting()`
  (công cụ giả lập DEBUG tự thêm 2026-07-16). Hai lỗi trong đúng 1 hàm:
  1. **Rải mẫu ở giờ NGẪU NHIÊN 9h-21h, 4-6 mẫu/ngày** → 2 mẫu bất kỳ cách nhau ~2-3 TIẾNG, vượt xa
     ngưỡng "quãng không gõ" (`vBellInterval × 1.5` = 22,5 phút) → view chèn `NSNull` giữa **mọi cặp**
     → mọi mẫu bị cô lập → `ampAt:` không nội suy được ở đâu cả → **không vẽ nổi một đoạn nước nào**.
     View đang tuân thủ chính xác dec.4 ("cấm bịa nước ở chỗ không có dữ liệu"); dữ liệu giả mới là
     thứ phi thực tế.
  2. **Đặt mẫu ở 9h-21h của HÔM NAY kể cả khi hiện tại mới ~1h sáng** → chấm nằm ở **tương lai**.
- **Vá:** giả lập theo **PHIÊN GÕ** thay vì rải điểm — trong phiên, mẫu cách đều **đúng
  `vBellInterval`** (900s < ngưỡng 1350s → nước liền mạch **theo cấu trúc**, không may rủi); giữa các
  phiên mới là quãng trống thật. Biên độ **đi bộ dần** (±0.10/nhịp, kẹp 0.05..0.85) thay vì nhảy cóc
  0.1↔0.8 mỗi 15 phút → sóng có hình dâng/lắng đọc được, đúng thứ cần để kiểm "sông có bám trạng thái
  không". **Không bao giờ ghi mẫu ở tương lai**; riêng HÔM NAY neo phiên **kết thúc đúng lúc này**
  (nếu neo theo khung 9h-21h thì test lúc 1h sáng sẽ ra ngày trống trơn).
- **Bài học (đắt, lặp lần 2 trong 1 đêm):** dữ liệu giả **không giống thật** thì mọi nghiệm thu dựa
  trên nó đều vô giá trị — tệ hơn: nó **vu oan cho code đúng**. Lần 1 (đêm 2026-07-16) chính seed này
  lôi ra crash `[NSNull doubleValue]` có thật; lần 2 nó tạo triệu chứng giả khiến chủ dự án tưởng
  dòng sông hỏng. Công cụ giả lập phải mô phỏng **cơ chế** (phiên gõ + nhịp chuông), không phải rắc
  số ngẫu nhiên cho "có dữ liệu".
- **⚠️ Việc kéo theo cho người test:** dữ liệu giả **CŨ** (rải rác) vẫn nằm trong kho — phải bấm
  **"[DEV] Xóa dữ liệu giả lập" TRƯỚC**, rồi mới "[DEV] Giả lập 30 ngày". Không xoá thì cũ + mới lẫn
  nhau và sông vẫn đứt.
- **Còn treo, chờ mắt chủ dự án phán sau khi thấy bản đúng:** chiều cao mỗi CHẤM = `midY - amp ×
  maxWaveH × sin(x × 0.19)` — tức phụ thuộc chấm rơi vào đâu trong chu kỳ sóng (~33px), **không**
  phải đọc thẳng biên độ của chính nó. Có dòng liền mạch thì chấm "cưỡi sóng", đọc được bình thường
  (biên độ nằm ở ĐƯỜNG BAO, đúng thiết kế). Nhưng nếu sau khi vá chủ dự án vẫn thấy chấm "cân chỉnh
  không rõ" thì đây là chỗ phải mổ tiếp — chạm nhận diện, không tự đổi.
- **Evidence:** build sạch · `make test` 16/16 · `brand-lint` 0 · `git diff core/` rỗng · đã cài
  `/Applications`. Tính liền mạch **chứng minh bằng số học** (900s < 1350s), KHÔNG phải bằng mắt —
  kho mã hoá không đọc được từ ngoài, menu [DEV] không lái được bằng script. **Cần chủ dự án bấm.**

---

### 2026-07-16 — Trả 3 món "nợ đã biết": gộp 3 đồng hồ · icon chuông hết tráo · "Độ nhạy" thôi nói dối
- **Bối cảnh:** chủ dự án chốt kế hoạch hoàn thiện (chặng A→F), yêu cầu khắc phục chặng **E — nợ đã biết**.

**1. BA đồng hồ trôi lệch → MỘT nhịp.**
- Trước: `g_bellTimer` (BellMac, reo tiếng) · `g_checkinTimer` (PanelViewController, hiện khung chấm
  nhịp) · `g_sampleTimer` (MoodWatchMac, ghi điểm lên sông) — cả 3 cùng lấy `vBellInterval` nhưng
  **mốc bắt đầu là lúc chủ của nó khởi động**, nên trôi lệch nhau vài phút. Trong khi màn Chuông hứa
  thẳng: *"Một nhịp, hai vai."*
- Nay `g_bellTimer` là **nhịp gốc**, bắn `kMKMoodBeatNotification`; 2 nơi kia **lắng nghe** thay vì tự
  đếm. Xoá hẳn `g_checkinTimer`/`g_checkinLastTime`/`g_sampleTimer`/`g_sampleLastTime`.
- **Cái bẫy suýt dính:** `BellMac_ApplySettings()` có `if (!vBell) return;` — tắt chuông là KHÔNG tạo
  đồng hồ. Gộp ẩu vào đó = **tắt chuông thì tắt luôn nhật ký + khung chấm nhịp**, tức âm thầm tắt việc
  ghi nhận khi người dùng chỉ muốn yên tĩnh (hành vi cũ: 2 đồng hồ kia CHƯA BAO GIỜ phụ thuộc `vBell`).
  Vá: bỏ `return` đó, **nhịp luôn chạy**; cổng chặn TIẾNG chuyển vào trong `bellTick()`. Kiểm thêm:
  `BellMac_NextRingDate`/`MinutesUntilNextRing` tự có check `!vBell` riêng nên dòng "Dự kiến reo lúc"
  không bị ảnh hưởng.
- **CỐ Ý giữ lại 1 đồng hồ 60 giây** (`g_bellLineTimer`): nó chỉ cập nhật dòng đếm ngược "chuông kế
  tiếp: còn N phút". Đó là nhịp **hiển thị**, không phải nhịp của app — gộp nốt vào nhịp 15 phút sẽ
  làm số đếm ngược đứng hình 15 phút một lần.
- Luồng ghi mẫu vẫn về đúng `g_moodQueue` trước khi chạm `g_sampleSum`/`g_sampleCount` (2 biến này chỉ
  được đọc/ghi ở hàng đợi đó) — không đổi mô hình luồng, chỉ đổi nguồn kích hoạt.

**2. Icon chuông tráo tên → sửa ẢNH, không sửa ánh xạ.**
- `bell_chime` (hình chuông cầm) đang phát **Chuông gió**; `bell_wind` (hình chuông gió) đang phát
  **Chuông reo**.
- **Bằng chứng tên file ĐÚNG (đo bằng `afinfo`, không nghe đoán):** Chuông chùa **23.8s** (ngân dài) ·
  Chuông gió **10.3s** (leng keng lâu) · Chuông reo **2.0s** (ngắn gọn). Độ dài khớp đúng tên → **ảnh sai**.
- Nên đổi ảnh (tag 1 → `bell_wind`, tag 2 → `bell_chime`), **KHÔNG** đổi ánh xạ tag→tên: đổi ánh xạ sẽ
  khiến `vBellSoundName` đã lưu của người dùng đột nhiên phát ra tiếng khác. Đổi ảnh = tiếng giữ
  nguyên, chỉ hình về đúng chỗ, không cần migration.

**3. "Độ nhạy" thôi nói dối.**
- Caption ghi *"dùng chung cho **nhật ký lấy mẫu** và chuông"* — **sai**: tra hết nơi dùng thì nó chỉ
  chạm chuông (chuỗi câu căng liên tiếp). Nó KHÔNG đổi cách lấy mẫu (điểm ghi lên sông vẫn là risk thô
  trung bình, không qua ngưỡng nào).
- Tệ hơn, **lỗi do chính phiên này gây ra sáng nay**: `kMoodRippleThreshold` cắm cứng **0.4**, nên câu
  "Sáng và chiều có gợn" phớt lờ lựa chọn của người dùng. Để "Ít nhạy" (0.6) thì **chuông im mà chữ
  vẫn nói có gợn**.
- Vá: `kMoodRippleThreshold` (hằng số) → `MoodPhrasing_RippleThreshold()` (đọc lúc chạy), uỷ thác cho
  `NudgeCoordinatorMac_RippleThreshold()` — nơi ĐÃ đọc `vBellSensitivity` cho chuông từ story 1.5.
  `ReflectionScreenMac` bỏ `static const` theo (giá trị nay đọc lúc chạy, không còn là hằng số biên dịch).
- Caption viết lại cho đúng sự thật MỚI: *"dùng chung cho chuông và các câu quan sát. **Không đổi ngưỡng
  gác cổng**."* — câu cuối cố ý: ngưỡng gác cổng đóng cứng 0.5 trong `core/`, người dùng rất dễ tưởng
  chọn "Nhạy" là gác cổng chặn sớm hơn. Không hề.
- **Còn treo:** "Độ nhạy" vẫn KHÔNG chạm được (a) giá trị mẫu ghi vào nhật ký, (b) ngưỡng gác cổng.
  Nó điều khiển 2/4 nơi người dùng có thể tưởng nó điều khiển. Muốn nó thật sự chạm gác cổng thì phải
  sửa `core/` (đang đóng băng với đội iOS) → cần chốt riêng.

- **Made by:** platform-shell-agent (trực tiếp).
- **Evidence:** build sạch · `make test` 16/16 · `brand-lint` 0 (135 file) · `git diff core/` rỗng.
  Nhịp gộp verify bằng CHẠY THẬT: đặt `vBellInterval=1` (sàn Debug), chờ nhịp nổ, xem kho có được ghi
  thêm mẫu không + app còn sống không.

---

### 2026-07-16 — Khung chấm nhịp: ở lại tới khi chọn/bỏ qua (bỏ tự-đóng 8 giây) + nối nút "Bỏ qua" bị bỏ quên
- **Decision:** Chủ dự án: *"muốn thông báo này giữ lâu hơn hoặc chọn rồi mới tắt"*. Chốt phương án
  **ở lại tới khi người dùng CHỌN hoặc bấm BỎ QUA** (không phải chỉ kéo dài thời gian).
  - Bỏ hẳn `dispatch_after` tự đóng sau 8 giây (`PanelViewController.mm -showCheckinOverlay`).
  - **Thêm nút "Bỏ qua — app vẫn tự ghi nhịp này"** — câu chữ lấy nguyên artifact "Vòng Soi lại" chủ
    dự án đã duyệt (`.skip`, kèm chú thích "Một chạm, **tự nguyện**"). **Nút này chưa từng được code**
    dù artifact đã có — phát hiện lúc đối chiếu code với artifact. Style: link mờ tông `stone`, KHÔNG
    phải nút CTA (đây là lối thoát, không phải lời mời chào). Bỏ qua = **không ghi checkin**, không
    đếm, không nhắc lại (nhịp lấy mẫu tự động vẫn chạy riêng ở `MoodWatchMac` — đúng câu đã hứa).
  - Panel cao 90 → 116 để chừa hàng "Bỏ qua".
- **Rationale (chạm HIẾN CHƯƠNG — ghi rõ để không ai đảo ngược nhầm):** "ở lại tới khi chọn" mà
  **không có** lối thoát = cứ mỗi nhịp (sàn 15 phút) chặn người dùng, không đáp không cho đi → đó là
  **hối thúc**, thứ hiến chương cấm, và trái chính artifact đã duyệt ("tự nguyện"). Có "Bỏ qua" thì
  khung nằm chờ = **tôn trọng** (không biến mất lúc người ta đang nghĩ). Nên nút Bỏ qua KHÔNG phải
  tính năng phụ — nó là **điều kiện** để khung được phép ở lại. Ai bỏ nút này phải trả lại tự-đóng-theo-giờ.
- **2 hệ quả kỹ thuật phải xử kèm (không có thì "ở lại" đẻ bug mới):**
  1. **Chống xếp đống:** trước đây mỗi nhịp dựng 1 panel mới, không kiểm cái cũ (vô hại vì 8 giây tự
     tan). Nay giữ `g_checkinPanel` — nhịp sau tới thì **thay** khung cũ. Tức khung cũ tự tan đúng lúc
     nhịp mới tới: thời hạn buông tay **bám nhịp lấy mẫu**, không phải con số hết-giờ tuỳ tiện. Không
     có cái này thì rời máy 2 tiếng = về bàn thấy 8 khung đè nhau.
  2. **`hidesOnDeactivate`:** `NSPanel` mặc định YES. Bản cũ tự đóng sau 8 giây nên không ai thấy hệ
     quả; nay khung phải sống tới khi người dùng chạm, mà người dùng LUÔN đang gõ ở app khác (đây là
     bộ gõ) → để mặc định thì khung lặng lẽ biến mất, phá đúng thứ vừa chốt. Tắt hẳn, cùng cách
     `SendGatekeeperMac.mm` đã làm cho overlay nhịp thở.
- **Thêm `[DEV] Hiện khung chấm nhịp ngay`** (`AppDelegate.m`, `#if DEBUG`): khung chỉ tự hiện mỗi
  `vBellInterval` phút (sàn 15) → không có lối gọi tay thì mỗi lần sửa phải chờ 15 phút mới thấy, nên
  **không ai verify** — đúng cách bug lọt (bài học 2026-07-16). `showCheckinOverlay` là method private,
  forward-declare trong `AppDelegate.m` (KHÔNG sửa `PanelViewController.h`) — cùng idiom
  `SettingsWindowController.mm` đã dùng cho `ConvertToolViewController -fillData`.
- **Made by:** platform-shell-agent (chạm nhận diện → đã hỏi chủ dự án trước, chốt qua AskUserQuestion).
- **Evidence:** build sạch · `make test` 16/16 · `brand-lint` 0 (130 file) · `git diff core/` rỗng ·
  đã cài `/Applications`, app đang chạy. **CHƯA verify được bằng máy** — status item cố ý KHÔNG gắn
  menu (bấm trái = popover, bấm phải = menu dựng tay) nên AppleScript không mở được menu để bấm mục
  [DEV]; synthetic right-click cũng không mở. **Cần chủ dự án tự bấm.** KHÔNG tuyên bố "đã chạy tốt".

---

### 2026-07-16 — Chẩn "UI lệch" = nhà cải tạo dở; lập Screen Reference + hợp đồng linh kiện
- **Decision:** Chủ dự án gửi 5 ảnh cửa sổ Cài đặt (Hôm nay/Chuông/Bộ gõ/Riêng tư/Hệ thống) báo "UXUI lệch rất
  nhiều". Soi code: gốc bệnh KHÔNG phải làm ẩu từng màn mà là **hai thế giới dán nhau** — pane brand mới
  (Hôm nay/Chuông/Riêng tư/Giới thiệu) vs **pane storyboard OpenKey cũ chưa migrate** (Bộ gõ + Hệ thống, qua
  `tabviewPrimary`/`tabviewSystem` trong `SettingsWindowController.mm`). Hệ quả: toggle Bộ gõ nhợt/lệch =
  control cũ; **"Hệ thống" trắng** = hộp cũ rỗng; Chuông 15/30/60 trôi khỏi decision 2026-07-15 (30/60+Tùy chỉnh).
  Lập **`bmad-output/macos/SCREEN-REFERENCE.md`** = 3-trong-1: spec build + nguồn user guide + hợp đồng linh kiện
  (Phần 0: PHẢI dùng `BrandControls`/`BrandColors`, CẤM màu thô / `NSButton` toggle / pane storyboard cũ). Kèm
  bộ mockup chuẩn 6 màn (session này). Chủ dự án chọn: **làm cả tài liệu + mockup**; **hoãn** cổng lint tự động (spec trước).
- **Rationale:** sơn lẻ từng màn chỉ đẻ ra phòng thứ 6 lệch kiểu mới; tuân thủ đến từ **bỏ bớt tự do + cổng chặn**,
  không từ lời nhắc. Một tài liệu phục vụ cả 3 nhu cầu (sửa UI, ép agent, viết hướng dẫn) là đòn bẩy cao nhất.
- **Còn mở (Phần 3 tài liệu):** nội dung "Hệ thống" (4 mục đề xuất); xưng hô toàn app "bạn"; tên 3 bộ tiếng chuông.
- **Made by:** hội thoại chủ dự án + recon code (session 2026-07-16)
- **Supersedes:** không (bổ sung; DESIGN/EXPERIENCE-macos-control-panel mô tả bố cục cũ vẫn cần soát riêng)

### 2026-07-16 — Cải tiến 3 màn cảm xúc + chốt hướng "ô ghi cảm nhận" cuối ngày
- **Decision:** Từ phản hồi chủ dự án ("chưa hiểu màn Hôm nay / Soi lại"), chốt một loạt cải tiến trên
  ...
  2. **Màn Soi lại v2.1** (chưa code — gói thành lệnh cho `platform-shell-agent`): câu hỏi + gợi ý phải
     ...
     **giờ đỉnh thật** (bỏ hardcode 15-17h) và **hạ xuống link cam nhẹ** (không phải nút cam đặc) để "câu
     hỏi giữ vai trung tâm; xưng hô thống nhất **"bạn"** (đang lẫn "anh"); seed câu **theo ngày** (không đổi mỗi lần mở).
  3. **Ô ghi cảm nhận cuối ngày (daily note)** — chốt HƯỚNG, chưa code: xem `_shared/DECISION-daily-note-v1.md`.
- **Rationale:** (1) sông cũ chỉ vẽ nước ở chỗ có mẫu nên sáng/chiều thành 2 khúc rời — trục nét đứt nối
  cả ngày mà không phạm luật "không bịa nước giả". (2) câu hỏi random có thể cãi cái quan sát ngay trên nó;
  nút cam đặc kéo mắt khỏi câu hỏi. (3) ô ghi là mảnh còn thiếu của "tự nhận ra", NHƯNG là lần đầu app lưu
  **chữ thật** của người dùng (trước chỉ số suy ra) → bậc riêng tư mới, phải chốt schema + consent riêng +
  luật copy + đồng bộ iOS TRƯỚC khi code, không lặng lẽ thêm cột. Chủ dự án chọn: nút chuông = link cam nhẹ;
  ô ghi = soạn quyết-định-thiết-kế trước.
- **Made by:** hội thoại chủ dự án + mockup 3 màn (session 2026-07-16)
- **Supersedes:** không (bổ sung cho entry 2026-07-13 "Diện mạo mới v2" + 2026-07-16 "Story 3.6 v2")

### 2026-07-16 — Vá crash thật: MKRiverCanvas chia cho 0 khi vẽ sông có dữ liệu lần đầu
- **Decision:** Chủ dự án báo "không mở được ứng dụng nữa" sau khi bấm nhầm nút "Kết thúc" ở màn
  fallback cũ (xem entry ngay dưới) rồi thử mở lại. Điều tra qua `~/Library/Logs/DiagnosticReports/`
  (KHÔNG đoán) — xác nhận 4 crash report GIỐNG HỆT NHAU, cùng 1 điểm nổ: `-[MKRiverCanvas
  drawRect:]`, `EXC_BREAKPOINT`. Nguyên nhân: `EmotionRiverView.mm` dòng vòng lặp vẽ sông tính
  `f = (x/w)*(k-1)` — nếu view bị vẽ lúc `bounds` rộng 0 (1 lần layout sớm/thoáng qua nào đó),
  `x/w = 0/0 = NaN`; ép NaN sang `NSUInteger` là hành vi không xác định, ra chỉ số vượt quá
  `_samples.count` → `NSRangeException` không ai bắt giữa chu kỳ vẽ của AppKit → crash CẢ APP.
  Lỗi này tồn tại từ lúc sông được code (2.4/3.5-3.8) nhưng CHƯA TỪNG lộ vì `k` (số mẫu) luôn = 0
  trong mọi lần test thật trước đây (return sớm trước khi chạm phép chia) — chỉ lộ ra NGAY TỐI
  NAY vì công cụ giả lập dữ liệu (entry dưới) lần đầu tạo ra `k > 0` thật. Vá: thêm `if (w <= 0)
  return;` (giống hệt guard `k == 0` đã có) + kẹp cứng chỉ số vào `[0, k-1]` làm lớp phòng thủ thứ 2.
- **Rationale:** Đây là bug thật, không phải cấu hình/quyền — xác nhận bằng crash log (backtrace
  giống hệt nhau 4 lần), không phải suy đoán. Ghi riêng vì đây là ví dụ cho thấy 1 tính năng "chưa
  bao giờ crash" có thể ẩn 1 lỗi tiềm tàng đơn giản vì đường dữ liệu thật (k>0) chưa từng được đi
  qua trong test — bài học cho các view vẽ tay khác dùng phép chia theo kích thước view.
- **Made by:** platform-shell-agent (trực tiếp, đọc crash log hệ thống + code, không qua orchestrator).
- **Evidence:** build sạch · `make test` 16/16 · `brand-lint` 0 (130 file) · `git diff core/` rỗng
  · app relaunch, còn sống > 10 giây, không crash report mới nào xuất hiện sau bản vá (báo cáo cuối
  cùng lúc 00:57, TRƯỚC khi build vá xong). Chủ dự án CHƯA tự bấm lại đúng luồng đã crash để xác
  nhận — cần xác nhận bằng tay trước khi coi là xong hẳn.

---

### 2026-07-16 — Công cụ DEBUG-only giả lập dữ liệu sông (test Tuần/Tháng không cần chờ dùng thật)
- **Decision:** Chủ dự án hỏi có thể can thiệp dữ liệu ghi nhận để giả lập không (nhằm test hiển
  thị sông Tuần/Tháng ngay, khỏi chờ 7-30 ngày dùng thật). Vì đụng "riêng tư dữ liệu cảm xúc" (gate
  an toàn của mindful-key), đưa 3 lựa chọn qua AskUserQuestion — chủ dự án chọn **"Giả lập có đánh
  dấu riêng"**: ghi thẳng vào kho thật (`mood.enc`, kiến trúc chỉ có 1 kho, không có kho nháp) nhưng
  đánh dấu ẩn ở cột `app_bundle_id` (không dùng cho event `'sample'` — tái dùng cột có sẵn, KHÔNG
  `ALTER TABLE`/migration trên file mã hóa đang chạy thật). Thêm 3 hàm **CHỈ tồn tại trong build
  Debug** (`#if DEBUG`, xác nhận `GCC_PREPROCESSOR_DEFINITIONS` chỉ có `DEBUG=1` ở config Debug,
  KHÔNG có ở Release — biến mất hoàn toàn khỏi bản phát hành):
  `MoodStoreMac_SeedFakeSamplesForTesting(numDays)` (4-6 mẫu/ngày, giờ 9h-21h ngẫu nhiên, risk
  0.10-0.79), `MoodStoreMac_DeleteSimulatedData()` (xóa ĐÚNG phần đánh dấu, dữ liệu thật không đụng),
  `MoodStoreMac_HasSimulatedData()`. Wire 2 mục menu tray `"[DEV] Giả lập 30 ngày dữ liệu sông"` +
  `"[DEV] Xóa dữ liệu giả lập"` (cũng `#if DEBUG`, `AppDelegate.m`).
- **Rationale:** Không chọn "sổ nháp riêng tách biệt" (an toàn tuyệt đối hơn) vì kiến trúc hiện tại
  không có khái niệm 2 kho — thêm đường dẫn thứ 2 là thay đổi kiến trúc lưu trữ, ngoài phạm vi 1
  việc test nhỏ. Không chọn "giả thẳng + xóa sạch sau" vì `MoodStoreMac_DeleteAll()` xóa TOÀN BỘ kể
  cả dữ liệu thật đã có — đánh dấu riêng cho phép xóa CHỌN LỌC, an toàn hơn mà tốn thêm rất ít code
  (tái dùng cột có sẵn, không schema migration).
- **Made by:** platform-shell-agent + mood-layer-agent (trực tiếp).
- **Evidence:** build sạch (Debug) · `make test` 16/16 + iOS build-smoke OK · `brand-lint` 0 (130
  file) · xác nhận qua `project.pbxproj`: `DEBUG=1` chỉ xuất hiện ĐÚNG 1 lần, gắn với config Debug
  · `git diff core/` rỗng · app đã restart. Chủ dự án CHƯA tự bấm thử — cần xác nhận bằng tay.

---

### 2026-07-16 — Chủ dự án nhìn app thật lần đầu (gate 3): tìm ra 1 bug hiển thị + gửi thiết kế mới cho màn Soi lại
- **Decision:** Chủ dự án gửi 2 ảnh chụp thật (popover + cửa sổ Cài đặt) — lần đầu có mắt người
  thật nhìn bản build của gate 3 (3.5-3.8, xem entry 2026-07-15). Kết quả:
  1. **Bug thật, không phải cảm nhận:** `riverTop` trong `mk_buildTodayPane`
     (`SettingsWindowController.mm`) lặp NGUYÊN công thức `segTop`, quên trừ chiều cao segmented
     "Ngày/Tuần/Tháng" (`kDateRangeSegH + kTodaySectionGap`). Vì `EmotionRiverView` (140pt) được
     add SAU segmented nên vẽ ĐÈ LÊN nó — control vẫn tồn tại trong view hierarchy, nhưng bị che
     kín hoàn toàn, khớp đúng ảnh chụp (không thấy Ngày/Tuần/Tháng đâu cả). Vá bằng 1 dòng.
  2. **Khoảng trống thiết kế, không phải bug:** chủ dự án kèm link artifact claude.ai
     "Mindful-key — Vòng Soi lại (chuông → sông → câu hỏi)" và nói màn Soi lại "vẫn là pop-up cũ".
     Đối chiếu code thật với artifact: đúng — story 3.6 (2026-07-15) mới chỉ đạt phần CHỨC NĂNG
     (sông thật, gỡ số thô) nhưng cấu trúc/văn phong vẫn là bản nháp 4 nhịp đơn giản, thiếu hẳn:
     đường ngăn giữa nhịp, nhãn mục kiểu eyebrow, đoạn quan sát tường thuật 2 câu (artifact gộp
     "đỉnh gợn vào buổi nào" + "quãng lặng dài nhất vào buổi nào" thành 1 đoạn, bỏ hẳn dòng số thô
     "Quãng lặng dài nhất: N phút" — đổi sang mô tả buổi, không số), caption phụ dưới câu hỏi, thẻ
     gợi ý tách nền + nút "Để sau", chân trang 3 dòng tin cậy, tiêu đề cửa sổ có thứ+ngày thật.
     Viết lại `ReflectionScreenMac_Show()` theo đúng artifact — 3 nhịp Nhận ra/Soi/Nuôi dưỡng thay
     4 nhịp cũ (nhịp "Cho phép" không còn là mục riêng, nội dung gộp vào đoạn quan sát ở nhịp 1).
  3. **Đổi hợp đồng phụ (thêm, không sửa):** `EmotionRiverView` thêm method MỚI
     `setCaptionHidden:` để `ReflectionScreenMac` tự viết câu quan sát riêng mà không bị caption
     tự sinh của view chồng lên — không đổi `setSamples:`/`preferredHeight` đã khoá từ 2.4. Tiện
     thể làm ấm câu caption trống-dữ-liệu mặc định (dùng chung ở popover lẫn cửa sổ Cài đặt) từ
     "Hồ chưa đủ nét — ngày mới bắt đầu." sang tông gần artifact hơn: "Hôm nay bàn phím nghỉ nhiều
     — và điều đó cũng chẳng sao."
  4. Layout của `ReflectionScreenMac_Show()` viết lại theo mẫu "cộng dồn khoảng cách TỪ ĐỈNH
     xuống, lật toạ độ qua ĐÚNG 1 hàm `ReflY()`" — chủ đích tránh lại đúng lớp lỗi vừa gặp ở mục 1
     (2 công thức tưởng giống nhau nhưng lệch do quên 1 số hạng).
- **Rationale:** Bug ở mục 1 là lỗi thật (đã verify bằng đọc code, không đoán). Khoảng cách ở mục
  2 không phải lỗi code — story 3.6 đã làm đúng Acceptance Criteria được viết lúc đó; artifact mới
  là input MỚI chủ dự án cung cấp sau khi tự trải nghiệm, nên coi đây là 1 vòng lặp thiết kế hợp
  lệ (không phải "làm sai từ đầu"). Tái dùng token `[Brand orangeLight]` có sẵn cho nền thẻ gợi ý
  (cùng ngoại lệ "brand chrome cho khoảnh khắc con người" đã dùng ở `SendGatekeeperMac`) thay vì
  thêm màu mới — giữ đúng "1 nguồn màu" của brand-lint.
- **Made by:** platform-shell-agent + mood-layer-agent (trực tiếp, việc rõ 1 mảng
  `platforms/apple/macos/`, không qua orchestrator).
- **Evidence:** build sạch · `make test` 16/16 (+ iOS build-smoke OK) · `brand-lint` 0 vi phạm
  (130 file) · `git diff core/` rỗng · app đã restart bản vá. **CHƯA ai nhìn bản đã vá** — gate 3
  giữ nguyên `review`, KHÔNG lên `done` cho tới khi chủ dự án tự xác nhận bằng mắt lần nữa.
- **Supersedes:** phần "build-verified" của 2026-07-15 "Đóng Epic 3 gate 3" (entry ngay dưới) cho
  riêng 2 mục Story 3.6 (viết lại cấu trúc) và 3.7/3.8 (vá hiển thị) — 3.5 không đổi.

---

### 2026-07-15 — Đóng Epic 3 gate 3: cả 4 story 3.5-3.8 code xong, chuyển `review`
- **Decision:** Sau khi hỏi chốt 2 câu hỏi thiết kế của 3.8 (entry dưới), triển khai code CẢ 4
  STORY trong cùng 1 lượt, đúng thứ tự `scope-conflict-check.sh` đã khoá:
  1. **3.5** — mount `GatekeeperCardView` vào pane "Hôm nay" cửa sổ Cài đặt, luôn trên cùng.
  2. **3.6** — `ReflectionScreenMac` vẽ sông thật (tái dùng `EmotionRiverView`), gỡ số thô
     "Đỉnh gợn hôm nay: 0.73" thay bằng câu mô tả buổi dùng `peakTs` (dead value từ 2.4, cùng
     lớp bug F14).
  3. **3.7** — `MoodStoreMac_FetchWeekSamples()` mới + segmented "Ngày/Tuần" (`MKDateRangeSeg`,
     view riêng — không đụng `MKSegmented` của `BellSettingsView.mm` đang bất ổn từ phiên khác).
  4. **3.8** — `MoodStoreMac_FetchMonthSamples()` (dùng chung `FetchDailyAverages(numDays)` với
     3.7 — viết 1 lần vì cả 2 code cùng lượt) + segmented đủ 3 mục Ngày/Tuần/Tháng.
  Cả 4 file story đã cập nhật `Status: done` + Dev Agent Record. Epic 3 gate 3 → `sprint-status.yaml`
  chuyển `sharded` → `review` (KHÔNG `done` — chưa ai nhìn app thật, đúng luật bằng chứng Epic 3).
- **Phát hiện lúc code, khác với lúc viết story (ghi lại để minh bạch):**
  - `EmotionRiverView` chỉ có ĐÚNG 4 slot nhãn trục cố định — 3.7's AC4 đề xuất tên thứ trong
    tuần ("T2/T3/...", 7 tên) không nhét vừa. Đổi sang mốc tương đối 4 điểm ("7 ngày trước" →
    "Hôm nay"), áp dụng cùng kiểu cho Tháng. Vẫn đúng tinh thần AC4 gốc (không dd/mm, không cảm
    giác lịch), chỉ khác cách diễn đạt cụ thể.
  - Story 3.6: cửa sổ Soi lại giãn thêm đúng `riverH+12` để chèn sông — nhưng nhịp 2/3/4 phải
    giữ NGUYÊN vị trí tuyệt đối gốc (không cộng thêm phần giãn), vì 2 hiệu ứng (cửa sổ giãn +
    nhịp 1 ăn hết phần giãn đó) triệt tiêu nhau. Bản nháp đầu tính sai (cộng dư), phát hiện + sửa
    trước khi build.
- **Rationale:** Chủ dự án: "triển khai làm khắc phục luôn đi nhé, không bị sót nữa" — hỏi trước
  (2 câu 3.8) rồi chạy một mạch, thay vì để việc dở dang qua nhiều lượt hội thoại.
- **Made by:** platform-shell-agent + mood-layer-agent (qua lệnh triển khai trực tiếp)
- **Supersedes:** không. Đóng hoàn toàn phần "shard" của entry "Shard Epic 3 gate 3..." phía dưới
  — 4 story từ trạng thái kế hoạch (backlog/ready-for-dev) sang code xong (done/build-verified).

---

### 2026-07-15 — Chốt câu hỏi thiết kế Tháng (3.8) + triển khai code cả 4 story 3.5-3.8
- **Decision:** Chủ dự án: "triển khai làm khắc phục luôn đi nhé, không bị sót nữa." Trước khi
  code, hỏi lại đúng 2 câu 3.8 tự ghi "DỪNG-HỎI" trong chính nó (không đoán để khỏi phải hỏi lại
  sau):
  1. **Hình dạng Tháng:** giữ nguyên 1-đường-1-hue teal như Ngày/Tuần — KHÔNG cần dạng "nén theo
     tuần" (4 điểm thay 30).
  2. **Độ thưa dữ liệu:** chấp nhận sông "lỗ chỗ" nhiều gap thật — KHÔNG cần ngưỡng "≥N ngày mới
     hiện" kiểu "Hồ chưa đủ nét" của Ngày.
  Cả 2 đều chọn phương án AN TOÀN đã đề xuất sẵn trong story. 3.8 chuyển `backlog` → `ready-for-dev`.
  Cả 4 story (3.5→3.6→3.7→3.8) triển khai TRONG CÙNG LƯỢT này, đúng thứ tự tuần tự đã khoá bởi
  `scope-conflict-check.sh`.
- **Rationale:** Nếu để tới lúc bắt tay code 3.8 mới hỏi thì đúng kiểu rủi ro "sót" mà chủ dự án
  vừa dặn tránh — hỏi TRƯỚC khi bắt đầu chuỗi thi công để có thể chạy 1 mạch không phải dừng
  giữa chừng.
- **Made by:** hội thoại trực tiếp với chủ dự án (2 câu hỏi xác nhận)
- **Supersedes:** đóng 3 câu hỏi thiết kế mở ở story `3.8.month-river-view.story.md` (ghi "DỪNG-HỎI").

---

### 2026-07-15 — Shard Epic 3 gate 3 thành 4 story đầy đủ (bmad-epics-and-stories)
- **Decision:** Chủ dự án yêu cầu "lên kế hoạch triển khai phát triển" cho gate 3 (hồn của
  "Hôm nay": card Gác cổng + sông thật + Ngày/Tuần/Tháng) — dùng skill `bmad-epics-and-stories`
  thay vì tiếp tục ghi note tự do trong `sprint-status.yaml`, vì việc này đã phình đủ lớn/đủ rủi
  ro nhận diện để xứng đáng rigor cao hơn các gate nhẹ khác (1/2/4/5/6/7 vẫn giữ dạng note, không
  đổi). Kết quả: 4 story mới, `bmad-output/macos/stories/`:
  1. **`3.5.gatekeeper-card-and-reflection-link`** (ready-for-dev) — mount `GatekeeperCardView`
     (tái dùng 100%, đã chạy tốt trong popover) vào pane "Hôm nay" cửa sổ Cài đặt. NHỎ.
  2. **`3.6.reflection-screen-real-river`** (ready-for-dev) — vẽ lại "Nhận ra" của màn Soi lại
     bằng `EmotionRiverView` thật (thay NSTextField phẳng), gỡ số thô "Đỉnh gợn hôm nay: 0.73"
     (vi phạm "không nhãn số mặc định"), thay bằng câu mô tả buổi dùng `peakTs` — biến đã tính
     đúng từ story 2.4 nhưng **0 nơi dùng** (cùng lớp bug với F14 hôm qua). VỪA.
  3. **`3.7.week-river-view`** (ready-for-dev) — `MoodStoreMac_FetchWeekSamples` mới + UI
     Ngày/Tuần. LỚN, việc mới hoàn toàn.
  4. **`3.8.month-river-view`** (**backlog** — KHÔNG ready-for-dev) — `FetchMonthSamples` +
     UI Tháng. Giữ ở backlog vì AC của chính story này tự ghi "PHƯƠNG ÁN AN TOÀN, CHỜ XÁC NHẬN"
     — chưa có mockup/quyết định gốc nào tham chiếu cho "Tháng" (khác cả "Tuần", vốn ít nhất còn
     xuất hiện dạng minh hoạ trong mockup A2). Cắm cờ DỪNG-HỎI 3 câu hỏi thiết kế cụ thể trong
     Dev Notes của story, không tự quyết hình dạng hiển thị.
  `scope-conflict-check.sh` xác nhận 3.5/3.7/3.8 chồng `SettingsWindowController.mm`, 3.7/3.8
  chồng thêm `MoodStoreMac.h/.mm` — cả 4 PHẢI làm TUẦN TỰ (3.5→3.6→3.7→3.8), không song song.
  Retired story placeholder "3.3.today-pane-gatekeeper-river-reflection" (chưa từng có file),
  cập nhật `epics.md` + `sprint-status.yaml` (`epic-3.story_ids`, gate 3 → `status: sharded` +
  `shard_into`) khớp theo.
- **Rationale:** Đọc lại story 2.4 ("done") trước khi viết story mới lộ 2 sự thật quan trọng cần
  sửa lại hiểu biết trước đó: (a) `EmotionRiverView` đã nối dữ liệu thật ở CẢ popover lẫn cửa sổ
  từ 2.4 — không phải "chưa nối dây" như khung note cũ của gate 3 có thể đọc nhầm, chỉ là CHƯA ĐỦ
  MẪU; (b) "xem theo 7 ngày" CHƯA từng nằm trong 9 quyết định gốc 2026-07-13 — chỉ là minh hoạ
  mockup, nên "Tuần" (3.7) và "Tháng" (3.8) đều là mở rộng MỚI từ câu trả lời trực tiếp hôm nay,
  không phải điều đã duyệt từ trước — cần ghi rõ để không ai đọc ngược thành "đã chốt từ đầu".
  Tách 1 gate thành 4 story (thay vì giữ 1 story lớn) vì độ lớn/độ rủi ro nhận diện khác hẳn
  nhau — đúng "Sizing Rule" của skill (story 1 dev-day, việc lớn hơn phải chẻ, không thổi phồng
  1 story cho vừa).
- **Made by:** `bmad-epics-and-stories` skill (qua yêu cầu trực tiếp "lên kế hoạch triển khai
  phát triển luôn được không")
- **Supersedes:** không xoá quyết định nào — bổ sung chi tiết thi công cho gate 3 đã mở
  2026-07-15 ("Nghiệm thu tay v2 KHÔNG PASS → mở Epic 3").

---

### 2026-07-15 — Nghiệm thu tay lần 2: sampling không bug + F15 (bố cục Bộ gõ) vá + gate 3 phình thêm Tháng
- **Decision:** Chủ dự án tự chạy app (bản sau commit `ba07891`), gửi 3 ảnh + 3 vấn đề. Điều tra +
  xử lý từng việc:
  1. **"Biểu đồ chưa cập nhật" — KHÔNG phải bug.** `defaults read vn.gnh.mindfulkey` xác nhận
     `MoodStoreConsentGranted=1`, `vBellInterval=15` trên máy thật. Sampling timer nằm trong
     `MoodWatchMac_Init()` (gọi từ `applicationDidFinishLaunching`), độc lập hoàn toàn popover —
     KHÁC lớp bug F12 (check-in) đã vá hôm qua. Sông rỗng lúc chụp ảnh là hành vi ĐÚNG: mỗi 15
     phút chỉ ghi 1 điểm, cần ≥3 điểm (~30-45 phút gõ thật trải dài) mới vẽ — đúng quyết định
     riêng tư 2026-07-13. Còn 1 điểm CHƯA kết luận được (cần chủ dự án xác nhận thêm): chữ "Hồ
     chưa đủ nét" có thật sự bị cắt cứng ở đáy popover, hay chỉ cần cuộn xuống là thấy — chưa có
     câu trả lời tại thời điểm ghi entry này.
  2. **(F15) Bố cục 11 công tắc "Bộ gõ ▸ Kiểu gõ" lệch — xác nhận + đã vá.** Toạ độ storyboard
     thật: cột trái 6 hàng (y=196→24), cột phải 5 hàng (y=196→55) — dòng "Tắt tiếng Việt khi bộ
     gõ hệ thống khác tiếng Anh" trơ trọi ở đáy, không cặp bên phải. ĐÃ VÁ: nới nhãn
     `MKLblOtherLang` 300pt→436pt, đọc thành 1 hàng full-width riêng. Xem `sprint-status.yaml`
     epic3_acceptance gate 6.
  3. **"Có nút không dùng được" — soát cả 11, không có nút chết.** Mỗi biến UserDefaults tương ứng
     đều được engine/`OpenKey.mm` đọc lại ở ≥3 chỗ. Có 1 dây phụ thuộc thật (Kiểm tra chính tả TẮT
     khoá 3 nút khác) dễ đọc nhầm thành hỏng — đã giải thích, KHÔNG sửa (đúng thiết kế).
  4. **(Gate 3) Phình scope lần 2 — thêm THÁNG.** Chủ dự án chốt: màn "Hôm nay" phải xem được
     sông theo **Ngày + Tuần + Tháng**. Mockup A2 gốc chỉ duyệt Ngày+Tuần — Tháng là mở rộng có
     chủ đích qua câu hỏi trực tiếp, không phải suy diễn. Kiểm tra: `MoodStoreMac` hiện **0 hàm**
     lấy dữ liệu quá 1 ngày — đây là việc viết MỚI hoàn toàn (tầng đọc dữ liệu + tầng vẽ), không
     phải nối dây có sẵn như các fix trước. CHƯA bắt đầu code — quá lớn để làm chung với các fix
     nhỏ trong lượt này, cần tách thành story riêng.
- **Rationale:** "Check giúp tôi" (không phải "triển khai") → ưu tiên chẩn đoán rõ ràng, chỉ sửa
  phần đã xác nhận rõ nguyên nhân + rủi ro thấp (F15), hỏi trước khi động vào phần lớn/mơ hồ (gate
  3 mở rộng, cần xác nhận phạm vi Ngày/Tuần/Tháng trước khi viết — đã hỏi qua AskUserQuestion và
  nhận câu trả lời rõ ràng cho cả 2 việc).
- **Made by:** platform-shell-agent (qua yêu cầu "check giúp tôi các vấn đề" + 2 câu hỏi xác nhận)
- **Supersedes:** không.

...
| Container app | UIKit (Swift hoặc Obj-C, chưa chốt) | Onboarding 2 màn, đọc App Group heartbeat |
...
```yaml
targets:
  MindfulKey:          # macOS — GIỮ NGUYÊN, không đụng
    ...
  ...
  MindfulKeyiOS:
    ...
    settings:
      base:
        PRODUCT_BUNDLE_IDENTIFIER: vn.gnh.mindfulkey.ios
        IPHONEOS_DEPLOYMENT_TARGET: "16.0"
        CODE_SIGN_ENTITLEMENTS: ios/App/MindfulKeyiOS.entitlements
  ...
  MindfulKeyKeyboard:
    ...
    settings:
      base:
        ...
        CODE_SIGN_ENTITLEMENTS: ios/KeyboardExtension/KeyboardExtension.entitlements
```

Cả 2 target mới cần entitlement App Group giống nhau (`group.vn.gnh.mindfulkey` — tên đề
xuất, cần chủ dự án xác nhận Bundle ID thật). `HEADER_SEARCH_PATHS` trỏ
`$(SRCROOT)/../../core/engine` + `core/mood`, giống hệt target macOS hiện có.
